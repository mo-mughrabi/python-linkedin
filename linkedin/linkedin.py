...
class LinkedIn(object):
    ...
    def access_token(self, request_token=None, request_token_secret=None, verifier=None):
        ...
        * oauth_version
        ...
        if self._request_token is None:
            raise OAuthError(
                "There is no Request Token. Please perform 'request_token' method and obtain that token first.")

        if self._request_token_secret is None:
            raise OAuthError(
                "There is no Request Token Secret. Please perform 'request_token' method and obtain that token first.")

        if self._verifier is None:
            raise OAuthError(
                "There is no Verifier Key. Please perform 'request_token' method, redirect user to API authorize page and get the _verifier.")
        ...
        if (submitted_url is not None) or (title is not None):
            ...
            if submitted_url is not None:
                ...
                if submitted_image_url is not None:
                    submitted_image_url_element = builder.create_element_with_text_node("submitted-image-url",
                                                                                        submitted_image_url)
    ...
    def _do_normal_query(self, relative_url, body=None, method="GET", params=None):
        ...
        query_dict["oauth_signature"] = self._calc_signature(self._get_url(relative_url),
                                                             signature_dict, self._access_token_secret, method,
                                                             update=False)
